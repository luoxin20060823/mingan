...
def build_disposal(decision: FinalDecision, hits: list[HitDetail], policy: PolicySettings | None = None) -> DisposalSuggestion:
    ...
    action = _action_for(decision.risk_level, decision.category, policy.high_risk_category_values(), hits)
    messages = {
...
def _action_for(
    risk: RiskLevel,
    category: str,
    high_risk_categories: set[str] | None = None,
    hits: list[HitDetail] | None = None,
) -> PlatformAction:
    ...
    if risk == RiskLevel.HINT:
        if _only_low_confidence_generic(hits or []):
            return PlatformAction.PASS
        return PlatformAction.HINT
...
def _only_low_confidence_generic(hits: list[HitDetail]) -> bool:
    return bool(hits) and all("low_confidence_generic" in hit.flags for hit in hits)
